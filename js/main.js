...
async function getIpInfo() {
    try {
        const response = await fetch('http://api.ipstack.com/check?access_key=745f78a7cc0dc3c5b323e239b3e28547');
        const data = await response.json();
        
        document.getElementById('ip-info').innerHTML = `
            <div class="prompt">IP: <span style="color:var(--neon-cyan)">${data.ip || 'Unknown'}</span></div>
            <div class="prompt">Location: <span style="color:var(--text-main)">${data.city || ''}, ${data.country_name || 'Unknown'} <img src="${data.location?.country_flag || ''}" width="16" style="vertical-align: middle"></span></div>
